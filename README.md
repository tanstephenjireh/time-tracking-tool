...
- **Cost-Optimized AI Fan-Out**: Each unique calendar event is sent to the LLM exactly once, regardless of how many employees attended it. Once the AI determines the category and client, that single classification is "fanned out" (copied) into the `ProcessedEvent` table for every internal employee involved (both attendees and the creator). This ensures massive API cost savings and drastic reductions in token usage.
